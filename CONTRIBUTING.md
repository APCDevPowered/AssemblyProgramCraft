...
* 如果你不是开发团队的成员，你想提交代码。那么请按以下步骤Fork项目，然后进行改动，并提交Pull Request。
* 首先你需要一个Github账号。请转到Github的主页，然后根据提示注册。
* 然后转到[https://github.com/APCDevPowered/AssemblyProgramCraft](https://github.com/APCDevPowered/AssemblyProgramCraft)，点击右上角的Fork按钮。
* 等待Fork完成后你就在你自己的Repositories里面有了APC当前版本的一个副本。
* 然后你需要按照上面所述的步骤来下载、魔改、构建项目。只不过需要注意所有的地址里的APCDevPowered都应该换成你自己的名字（把所有地址换成你Fork的项目的地址）。
* 当你改动完成想提交代码到主库时，首先用你的Git客户端提交代码到你克隆的项目，注意写好相应的提交注释。
* 然后转到Github上的项目页面，并点击左侧角的New pull request按钮。
* 接下来根据提示建立一个PR（Pull Request）到主库，然后在主动联系或者等待一段时间，我们就会去审查代码，并提出任何此PR的问题。
* 如果什么问题都没有，而且也不需要其他额外的工作，那么我们就会同意此PR，你的代码就会合并到主库，下一个主要版本发布时就很可能会随其发布。
* 或者如果你找到一个BUG，也可以建立一个Issue，当然如果你有能力改动、修复代码，还是建议直接修改后提交Pull Request。但请注意需符合提交规则。
