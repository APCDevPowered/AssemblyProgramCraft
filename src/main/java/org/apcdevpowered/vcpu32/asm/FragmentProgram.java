...
public class FragmentProgram
{
    ...
    public ProgramPackage link(CompileContext context) throws LinkException
    {
        ...
        programPackage.data = IntUtils.castToPrimitiveArray(programData.toArray(new Integer[programData.length()]));
    ...
    private void linkString(DynamicArray<Integer> programData, CompileContext context)
    {
        for (Entry<OffsetWithDataPackage<String>, List<Integer>> entry : stringRequestListMap.entrySet())
        {
            ...
            Integer[] stringData = StringUtils.writeStringToIntegerArray(string);
