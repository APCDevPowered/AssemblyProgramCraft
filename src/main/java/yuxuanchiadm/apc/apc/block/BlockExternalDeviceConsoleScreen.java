...
public class BlockExternalDeviceConsoleScreen extends BlockContainer
{
    public BlockExternalDeviceConsoleScreen()
    {
        super(Material.rock);
        this.setBlockName("block_external_device_console_screen");
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess blockAccess, int x, int y, int z)
    {
        if(blockAccess.getBlock(x, y, z).equals(this))
        {
            ...
            if(direction == 0)
            {
                this.setBlockBounds(0.0F, 0, 0.0F, 1.0F, 1, 4F/16F);
            }
            else if(direction == 1)
            {
                this.setBlockBounds(12F/16F, 0, 0.0F, 1.0F, 1, 1.0F);
            }
            else if(direction == 2)
            {
                this.setBlockBounds(0.0F, 0, 12F/16F, 1.0F, 1, 1.0F);
            }
            else if(direction == 3)
            {
                this.setBlockBounds(0.0F, 0, 0.0F, 4F/16F, 1, 1.0F);
            }
        }
        else
        {
    ...
    {
        return AssemblyProgramCraftProxyClient.RENDER_TYPE_BLOCK_EXTERNAL_DEVICE_CONSOLE_SCREEN;
    }
}
